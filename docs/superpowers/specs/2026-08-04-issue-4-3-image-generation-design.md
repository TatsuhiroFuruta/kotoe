...
**実測（2026-08-04、実 API で 1024×1024・low・WebP90）：278.6 KB / 生成 28.9 秒。**
見積り 0.3 MB とほぼ一致したので、上のメモリ試算とクレジット試算はそのまま使える。
生成時間も `read_timeout` 150 秒に対して十分な余裕がある。
...
### 7. コストガードは4層。**穏やかなガードが先に効くように値を決める**

**OpenAI の spend limit には性質の違う2つがある**（設定時に一次情報で確認。当初この設計は
「予算設定は通知のみ」と誤って書いていた）：

- **Spend alert** … 通知のみ。「API traffic continues」
- **Hard spend limit** … `Enforce a hard limit` を ON にすると**実際に止まる**。
  `429` ＋ `organization_spend_limit_exceeded` / `project_spend_limit_exceeded`。
  ただし遅延があり "recorded spend can slightly exceed the configured amount"

| 層 | 値 | 効いたときに起きること |
|---|---|---|
| **アプリ側の1日上限** | 50 枚/日 | `generate` が **503 で即座に断られる**。ジョブは積まれず、**生成枠も消費されない** |
| Spend alert | $5 | メール通知のみ。遮断しない |
| Hard spend limit | **$20**（`Enforce` ON） | 429 → attempt が **failed**。枠は戻らない |
| 前払いクレジット | **$20**・オートリチャージ off | 残高切れ → 429 → **failed**。枠は戻らない |

**下2つを 50枚/日 の月額最悪値（$16.5）より上に置くのが要点。** 下に置くと、アプリ側の
穏やかなガード（枠を消費しない 503）に達する前に 429 が返り、生成枠だけ失う乱暴なほうの
失敗が先に起きる。$20 なら 429 は「アプリ側のガードをすり抜けるバグがあったとき」だけの
最後の砦になる。

**請求を止めているのは hard limit ではなく「前払い＋オートリチャージ off」。** カードから
自動補充される経路が無いので、残高を使い切れば止まるしかない。hard limit は遅延で少し
超えるので、その取りこぼしを残高が受け止める関係になっている。なお hard limit は毎月
リセットされるが前払いクレジットはリセットされないので、数ヶ月経つと残高のほうが先に効く。

Spend alert を $5 にするのは、遮断しない以上低く置いて損がなく、$5 が想定利用
（5人 × 3回 × 30日）ちょうどなので**想定を超えた瞬間に気づける**ため。

**この2つの 429 はリトライしない。** 上限を上げるか残高を足すまで必ずまた 429 になるので、
レート制限（Transient）と分けて `PermanentError` にする（`Images::Generators::Openai` の
`quota_exhausted?`）。分けないと「時間を置けば直る」と誤判定して `rate_limited` という
誤った理由で failed になる。
...
| 429（`*_spend_limit_exceeded` ＝ hard spend limit） | Permanent | `api_error` |
...
判定に使うエラー文字列は**実 API で確認済み**（2026-08-04、マージ前にローカルから実キーで採取）。
ポリシー違反は `400` ＋ `error.code: "moderation_blocked"` ＋
`error.type: "image_generation_user_error"` で返る。`content_policy_violation` は別系統で
返る可能性に備えた保険として残してあるが、実測はしていない。
**認識できない値は `api_error` に倒す**フォールバックを置いてあるので、取りこぼしても
ジョブは壊れない（ユーザーに理由が伝わらなくなるだけ）。
...
5. ~~ポリシー違反を意図的に起こし、実際の `error.code` / `error.type` の文字列を採取する~~
   → **マージ前にローカルから実キーで採取済み**（`moderation_blocked` /
   `image_generation_user_error`）。本番でやり直す必要はない
