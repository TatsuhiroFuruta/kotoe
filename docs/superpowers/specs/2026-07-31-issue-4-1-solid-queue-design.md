...
0. **enqueue がアプリの更新と同じトランザクションに乗る（最重要）。** 詳細は下の
   「トランザクションの一体性」を参照。これはインフラの都合ではなく**アプリの正しさ**の話で、
   分離を検討するときに最初に確認すべき論点。
1. **Neon では分離の利益が出ない。** 同一ブランチ内の 2 つのデータベースは同じコンピュート
   ...
   必要で、それは有料プランの話になる。
   なお **Neon の 0.5 GB / 100 CU-hours はプロジェクト単位**なので、同じプロジェクト内で
   データベースを 2 つ作っても枠は増えない（仕切り直すだけ）。プロジェクトごと分ければ枠は
   倍になるが、ポーリングを受けるキュー側が先に枠を使い切って停止し、その時点で enqueue も
   処理も不可能になるため、アプリ側に枠が残っていても意味がない。
...
### トランザクションの一体性（同居の最大の利点）

> この節は実装完了後に追記した。設計時はインフラの都合（Neon のコンピュート共有）ばかりを
> 見ていて、**アプリの正しさへの影響に気づいていなかった**。分離を再検討するときは、
> 上の 1〜3 よりも先にここを読むこと。

4-2 で書く生成の起動処理はこの形になる。

```ruby
ActiveRecord::Base.transaction do
  attempt.update!(status: :generating)
  GenerateImageJob.perform_later(attempt.id)
end
```

`ActiveJob::Base.enqueue_after_transaction_commit` は **`false`** で、これは ActiveJob 側の
既定値（`activejob-8.1.3.1/lib/active_job/enqueuing.rb:53`）。Kotoe では上書きしていない。
したがって **enqueue はこのトランザクションの中で実行される**。ジョブテーブルが同じ
データベースにあるため、`solid_queue_jobs` への INSERT も同じトランザクションに含まれる。

結果として、**トランザクションがロールバックすればジョブの行も一緒に消える**。

| | 同居（採用） | 別 DB |
|---|---|---|
| Attempt の更新が失敗した場合 | **ジョブも消える** | **ジョブだけが残る** |
| その後に起きること | 何も起きない（正しい） | 存在しない Attempt に対する生成ジョブが走る |

別 DB にすると、enqueue はアプリ側のトランザクションと無関係に独立してコミットされるため、
**孤児ジョブ**が生まれる。孤児ジョブは Attempt が見つからず失敗するが、
**生成回数は enqueue 時点で既に消費されている**（このプロジェクトのドメインルール：
「生成はジョブ enqueue 時に1日の上限を消費する。削除しても回数は戻さない」）。

つまり **ユーザーが枠を 1 つ失って何も得られない**という、Solid Queue を導入して防ごうとした
事態そのものが、別の経路で発生する。

**回避策はあるが、穴が移るだけ。** `enqueue_after_transaction_commit = true` にすれば
孤児ジョブは防げるが、今度は「コミットは成功したが enqueue の直前にプロセスが死ぬ」隙間が
生まれ、**ジョブが積まれないまま `generating` のまま固まった Attempt** が残る。
どちらの穴を選ぶかという話になり、**同居ならどちらも起きない**。

**4-2 での含意**：この保証に依存するので、生成の起動は必ずトランザクションで囲む。
また `enqueue_after_transaction_commit` を `true` に変更しないこと（変更すると上記の
別の穴が開く）。
