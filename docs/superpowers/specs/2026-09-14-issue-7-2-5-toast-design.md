...
**代償**：スワイプで消す、ホバーで一時停止、スタックの折り畳みアニメーション、promise トーストは持たない。Kotoe の用途（「保存しました」1 行）には要らないと判断した。
...
**種別は色だけで区別しない。** 色覚特性のあるユーザーに緑と赤の判別を要求しないため、アイコンと読み上げ用の接頭辞を併せて持つ（下記 6）。

### 3. 表示は最大 3 件。同じ文言の連打は抑制しない

`issues_backlog` のタスク「同じメッセージの連打を抑制するか決める」。**抑制しない**。4 件目が来たら一番古いものを押し出す。
...
ビューポートは `toasts.length === 0` のときも `<ol>` を描く。スクリーンリーダーはライブリージョンを DOM に見つけた時点で監視を始めるため、**リージョンと中身が同時に現れると読み上げを取りこぼす**。
...
**タイマーは `Map<number, ReturnType<typeof setTimeout>>` で持つ。** `clearTimeout` するのは次の 2 箇所で、**両方必要**。

1. 手動で閉じたとき
2. 4 件目に押し出されたとき

2 を忘れると、既に消えた通知のタイマーが数秒後に発火して無関係な id を消しにいく。
...
export function ToastViewport() {
  ...
  return (
    ...
    >
      {toasts.map((t) => (
        <li key={t.id} className="pointer-events-auto ...">...</li>
      ))}
    </ol>
  );
}
...
**フォーカスは奪わない。** トーストが出た瞬間に入力欄からフォーカスが飛ぶと、7-3 で描写文を書いている最中に操作不能になる。
