...
- どちらの窓も、通り抜けたときの結果は同じ（下記）。**塞がない理由は 2 つの窓で違う**ので分けて書く。
  - **コントローラ側**は、塞ぐコスト自体は安い。`start_generation` の `update!(status: :generating)` の**手前**にお題の再確認を足せばよく、そこは枠を消費する前なので `generating` が残らず、`failed` に落とす必要も `FAILURE_REASONS` の追加も 4-5 への依存も発生しない（`RecordNotFound` を投げれば既存の 404 にそのまま乗る）。それでも塞がないのは、**この窓が桁違いに小さく、しかも塞いでもジョブ側の窓が残るから**。被害の期待値がほとんど動かない
  - **ジョブ側**は下記のとおり、塞ぐと新しい失敗コードが要る。こちらが本題
