...
- ✅=完了（対応する GitHub issue がクローズ済み）。優先度マーカーとは別の軸なので併記する。
...
- **7-1 からの申し送り（着手時に必ず処理する）**：
  ...
  5. **`restoreFailed` からの回復導線を足す**。7-1 は 401 以外の失敗（通信断、Render の
     コールドスタート）でトークンを捨てず未ログイン表示にし、`RequireAuth` は
     「再読み込みしてください」と出すだけ。同じトークンでの再試行は恒久的に塞がれるので、
     **回復手段がリロードしかない**。`useAuth` に `retryRestore()` を足して再試行ボタンを
     置くか、`online` / `focus` で自動再試行するのが素直。
  6. **グローバルナビを書く前に `restoreFailed` の意味を確認する**。この状態では `status` は
...
- タスク：
  ...
  - [ ] **E2E①に必ず含める（7-1 の申し送り）**：
    - **ログイン済みのままガード付きページを「直接ロード／リロード」しても `/login` に飛ばされないこと。**
      7-1 で実際にこのバグを埋め込んでいる。`useSyncExternalStore` はハイドレーション中
      `getServerSnapshot`（=null）を返し、実値への切り替えは passive effect で起きる。React は
      子の effect を親より先に実行するため、`RequireAuth` が先に「未ログイン」と判断していた。
      **`<Link>` のクライアント遷移ではハイドレーションが走らないので、この経路は永遠に露出しない。**
    - **ログアウト後に同じトークンで叩くと 401 になり、トークンが自動で破棄されること**
      （2-1 の denylist との結合点）。
