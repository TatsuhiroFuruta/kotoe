...
### 🔵 0-6. スマートフォン実機から開発サーバーを開けるようにする
- 目的：**モバイル幅の確認を実機で行えるようにする。** Next.js は dev リソースへのクロスオリジン
  アクセスを既定でブロックするため、同じ Wi-Fi のスマホから `http://<MacのLAN IP>:3001` を開いても
  **アプリが動かない**。
- 依存：0-3
- **症状が実装バグに見えるのが厄介**：HTML は SSR されるので画面は出るが、`/_next/*` が
  ブロックされてクライアント JS が動かず、ハイドレーションが完了しない。結果として
  ...
  どちらも「実装が壊れている」ようにしか見えない。**切り分けは dev サーバーのログを見る**のが確実で、
  `⚠ Blocked cross-origin request to Next.js dev resource /_next/... from "<IP>"` が出る。

  **`curl` では再現しない。** ブロックの判定は `Origin` / `Referer` ヘッダで行われ、curl は既定で
  どちらも送らないため 200 が返る。7-2.5 の実機確認で実際にこれで誤判定した。再現させるなら
  `curl -H "Origin: http://<IP>:3001" -H "Referer: http://<IP>:3001/"` を付ける。
- 先に決めること：
  ...
  2. **`NEXT_PUBLIC_API_BASE_URL` をどうするか。** 現在は `localhost:3000` 固定で、スマホから見た
     ...
     API を使わないので確認できたが、7-3a 以降は一覧・詳細とも API に依存する。LAN IP を向ける
...
- **7-3b への申し送り**：生成画像は WebP で保存しているため、**ダウンロードURLには必ず
...
- **何が困るか**：生成を起動した画面が閉じられると、その挑戦の `id` を知る手段が無くなる。
  - 生成中は、起動した画面が手元に持つ `id` をポーリングして追う（7-3b）。**タブを閉じるとその `id` は失われる**
...
- **7-3b への申し送り**：`best_attempts` はページングを持たない。`meta` は `attempts` のページングだけを指す。
...
- **7-2 で決めたこと（以降の全画面が乗る前提）**：
  ...
  - **ナビには実在するルートしか置かない**（main は Vercel 本番を追跡するため）。
    `SiteHeader` にコメントで置き場所を残してある。7-3a が「探す」→ `/posts`、
...
### 🟢 7-2.5. フラッシュメッセージ（トースト）の共通部品
- 目的：**画面遷移を伴わない操作の結果**をユーザーに伝える。CLAUDE.md「メッセージ・エラー・i18n の
  責務」が定めている「フロントがトーストを出す」を実際に置く場所を作る。規約は 0-2 の時点からあるが、
  **実装する issue が無いまま 7-2 まで来てしまった**ため、ここで積む。
- 依存：7-2（デザイントークンと共通レイアウトが要る）
- **7-3b の前に置く理由**：7-3b の描写入力には「保存」（下書き作成）があり、**押しても画面が変わらない**。
  通知が無いと保存されたか分からず、ドメインの必守ルール（「保存」＝下書き／「画像を生成」＝ジョブ起動
  の 2 ボタン）の片方が無反応に見える。
- **7-2 のフォームエラーとは別物**。兼用しないこと。

  | | 置き場所と寿命 | 例 |
  |---|---|---|
  | フォームのエラー（7-2 で実装済み） | 入力欄の直下／フォーム上部に**留まる** | 「このメールアドレスは既に登録されています」 |
  | フラッシュメッセージ（この issue） | 画面の隅に出て**自動で消える** | 「保存しました」「お題を投稿しました」 |

  入力を直すために読む必要があるものが数秒で消えると、ユーザーは直しようがなくなる。
- 先に決めること：
  1. **依存を足すか自前で書くか**（CLAUDE.md により依存追加は事前確認が要る）。候補は `sonner` /
     `react-hot-toast` など。7-2 はフォームで依存を足さない判断をしているが、トーストは
     ポータル・スタック・自動消去・アニメーションと自前で書く量が多いので、**同じ結論になるとは限らない**。
     判断材料として gzip 後のサイズと、下の要件をどこまで満たすかを調べてから選ぶ。
  2. **成功以外の種別を持つか**（success / error / info）。7-3b の生成失敗をここで出すなら要る。
- タスク：
...
- 消費する予定の issue：7-3b（保存した／生成を開始した）、7-4（通報しました）、7-5（お題を投稿しました）、
  7-6（削除しました）、7-2.6（セッション失効）。
- 完了条件：画面遷移を伴わない操作の後に通知が出て、数秒で消える。7-3b が「保存」で使える。

### 🟢 7-2.6. セッション失効の通知と再ログイン導線
- 目的：**JWT の失効をユーザーに説明する**。今は理由がどこにも残らず、原因の分からない挙動になる。
- 依存：7-2.5（認証不要ページでの通知にトーストが要る）、7-5（ガード付きページが初めて存在する回）
- **JWT の有効期限は 24 時間固定**（`config/initializers/devise.rb`）。つまり**毎日必ず起きる**。
  「稀なケース」ではない。
- 今どうなっているか（7-2 時点）：`api.ts` が「Authorization を載せたリクエストの 401」を検知して
  トークンを捨てる。ここまでは正しいが、**理由がどこにも残らない**。

  | ユーザーがいる場所 | 今の挙動 |
  |---|---|
  | ガード付きページ（7-5・7-6） | `RequireAuth` が `/login` へ飛ばす。**説明なし**。「急にログイン画面に放り出された」としか見えない |
  | 認証不要ページ（7-3b のお題詳細など） | 飛ばさない（意図的）。ヘッダーが黙ってユーザー名から「ログイン」に変わり、**押した操作は静かに失敗する**。長い描写文を書いて「保存」を押した場合が最悪 |

- **`RequireAuth` は原理的に理由を知れない**。`auth.status === "unauthenticated"` に着いた時点では、
  「最初から未ログイン」なのか「たった今 401 で失効した」なのかを区別する情報が残っていない
  （`api.ts` が既にトークンを捨てている）。**区別するには状態を足す必要があり、これは設計変更**。
  7-2 で `unreachable` を足したときと同じ規模の検討が要るので、**brainstorming を通すこと**。
- 先に決めること：
  1. **失効をどこに記録するか**。`AuthProvider` に「このセッション中に 401 で失効した」を持たせるか、
     `api.ts` が理由を通知する仕組みを作るか。7-2 の `deriveAuthState()` は 4 状態の純粋関数なので、
     5 つ目の状態にするのか、状態の外に持つのかも含めて決める。
  2. **ガード付きページでの見せ方**。`/login?reason=expired` のようにクエリで運ぶか、状態で持つか。
     クエリにするなら **`safeNextPath()` と同様に値を検証する**（`?reason=` も攻撃者が書ける）。
  3. **入力中のデータを失わせない配慮が要るか**。描写文を書いている最中に失効した場合、
     ログインし直して戻ったときに内容が消えていると実害が大きい。要るなら保存先を決める。
- 補足：文言は**すでに用意されている**。`src/lib/auth/error-messages.ts` の `unauthorized`
  （「セッションの有効期限が切れました。もう一度ログインしてください」）。7-2 のコードレビューで
  「どこからも到達しない」と指摘された箇所だが、**余分なのではなく、出す機能がまだ無いだけ**。
- 完了条件：失効したとき、ガード付きページでも認証不要ページでも理由が伝わり、
  ユーザーが次に何をすればよいか分かる。

### 🟢 7-2.7. フロントエンドの前提整備（timeout・ボタンの共通化）
- 目的：**7-3a / 7-3b が乗る足場を先に敷く**。7-1・7-2 の申し送りのうち、画面を 1 つも作らずに
  片づくものをここに集める。元は 7-3 の一部だったが、7-3 が 20 ファイル規模になるため切り出した。
...
- **7-1 からの申し送り**：`apiRequest` には timeout も `AbortSignal` も無く、**無限に待つ**。
  Render の無料枠はスリープするので、放置後の初回アクセスでコールドスタートに約 1 分かかる。
  ポーリングを入れる 7-3b の前に、`AbortSignal.timeout()` 対応を足しておくこと。
- **7-2 からの申し送り 1（コールドスタート中はトップが無操作になる）**：有効なトークンを持つ
  再訪ユーザーが `/` を開くと `/api/me` が返るまで `status: "loading"` で、ヘッダーは
  プレースホルダ、ヒーローの CTA も出ない（CTA は `unauthenticated` ゲート）。
  コールドスタートでは 30〜60 秒この状態が続き、押せるものがロゴだけになる。
  timeout を入れれば `unreachable` に落ち、`SiteHeader` が既に持つ再試行・ログアウトが出る。
- **7-2 からの申し送り 3（主ボタンのクラス列の重複）**：primary が 5 箇所（`page` /
  `login` / `signup` / `site-header` / `require-auth`）、secondary が 3 箇所に複製されている。
  padding も `px-3 py-1.5` / `px-4 py-2` / `px-5 py-2.5` の 3 種に散っている。
  7-3a のページネーションとソートで `<Link>` 側がさらに増えるので、ここで畳む。
- タスク：
  - [ ] `apiRequest` に `timeoutMs`（既定 15 秒）を足し、呼び出し側の `signal` と合成する
  - [ ] `ApiTimeoutError` と、通信断とは別の専用文言（「起動中の可能性があります」）
  - [ ] `buttonClasses()` への集約（`variant` × `size`。文字サイズは持たせない）
  - [ ] `test/lib/api.test.ts` に timeout の検証を足す
- 完了条件：backend を止めた状態で 15 秒以内にヘッダーが `unreachable` に変わり、
  ログインフォームが通信断と区別された文言を出す。
- **この issue で作らないもの**：ナビの「探す」→ `/posts` と、トップの CTA「お題を探す」。
  `/posts` が実在しないうちにリンクを置くと本番で 404 になる（`SiteHeader` のコメント参照）。
  どちらも 7-3a で足す。
...
- Cloudinary の URL をフロントで組み立てるのはこの issue が最初。`image_public_id` から
  組み立てるヘルパを 1 箇所に置く（7-3b・7-4・7-6 が使い回す）。
- **ナビとトップの CTA をここで足す**（7-2.7 から持ち越し）。`SiteHeader` の中央に
  「探す」→ `/posts`、`page.tsx` の CTA に「お題を探す」。置き場所はどちらもコメントで残っている。
- **他人が入力した値を初めて表示する issue（XSS の注意）**：ここで `title` / `name`
  （`PostSerializer` / `UserSerializer`）を画面に出す。React は JSX 内の値を自動で
  エスケープするので、**普通に `{value}` と書く限り安全**。危険なのは、そのエスケープを
  迂回する 3 経路。
  1. **`dangerouslySetInnerHTML`** … `react/no-danger` を eslint で **error にしてある**ので
     CI が止める。踏みやすいのは「描写文の改行を反映したい」場面（7-3b）で、
...
- 完了条件：お題を検索・並び替え・ページ送りして探せる。

### 🟢 7-3b. お題詳細（/posts/[id]）＋描写入力＋生成ポーリング
- 依存：**7-3a**, 6-1, 4-2, 7-2.5（「保存しました」のトースト）
- タスク：元画像ヒーロー＋タイトル＋投稿者＋お気に入り、**ベスト再現（表彰台 3 枠）**、
  描写入力（「保存」／「画像を生成」の 2 ボタン）、みんなの挑戦グリッド（再現度順／新着順・
  ページング）、生成中／空／失敗の状態。
- **ドメインの必守ルール**：「保存」＝下書き（`status: draft`）の作成・更新、「画像を生成」＝
  ジョブ起動。生成が成功したら**即公開**（結果を見てから公開を選ぶ導線は作らない）。
- **生成ポーリング**：`POST :generate` が 202 を返したあと `GET /api/attempts/:id` を
  ポーリングして `generating → published / failed` を追う。ポーリング間隔と打ち切りの
  ロジックは純粋な関数／カスタムフックに切り出し、**そこにだけ Vitest** を書く
  （CLAUDE.md のテスト方針）。アンマウント時は呼び出し側の `AbortSignal` で止める
  （7-2.7 で合成できるようにしてある）。
- **6-1 からの申し送り**：`best_attempts` はページングを持たない。`meta` は `attempts` の
  ページングだけを指す。表彰台と一覧は同じ挑戦を含みうる（一覧から除くと `total_count` と
  OFFSET がずれる）。
- **4-3 からの申し送り**：生成画像は WebP で保存しているため、ダウンロード URL には必ず
  拡張子を付けること。
- **「保存」の成功通知は 7-2.5 のトーストを使う**。押しても画面が変わらないため、
  通知が無いと保存されたか分からない。
- **失効（401）の扱いは 7-2.6 に委ねる**。この issue で初めて「認証不要ページで認証必須の
...
- **7-2 からの申し送り（この issue が `RequireAuth` の最初の実利用になる）**：
  ...
  1 つも無い状態が続いている**。E2E（8-1）も 7-3b・7-4 依存なので、ここが初めての
...
- **この issue の完了後に 7-2.6（セッション失効の通知）へ進む。** ガード付きページでの見せ方を
  決める issue なので、ここで初めて実物で確認できるようになる。
- 完了条件：画面からお題を投稿できる。
...
- **踏みやすい罠（着手時に必ず確認）**：
  ...
  2. **`img-src` に Cloudinary が必要**（7-3a 以降）。`'self' blob: data:` のままだと画像が出ない。
...
## 進行順のまとめ（MVPの背骨）
`0-1 → 0-2 → 0-3 → 1-1 → 2-1 → 2-2 → 8-2a → 3-1 → 3-2 → 4-1 → 4-2 →（4-3）→ 5-1 → 5-2 → 6-1 → 6-3 → 4-4 → 7-1 → 7-2 → 7-2.5 → 7-2.7 → 7-3a → 7-3b →（7-4・7-5）→ 7-2.6 →（7-6）→ 8-1 → 8-2b`

- **7-2.5（フラッシュメッセージ）は 7-3b の前**。7-3b の「保存」は押しても画面が変わらないため、
  通知が無いと保存されたか分からない。
- **7-3 は 3 つに割ってある**。7-2.7（画面を作らない前提整備）→ 7-3a（一覧）→ 7-3b（詳細）。
  1 つにすると 20 ファイル規模の PR になり、「1 issue = 1 PR」と釣り合わないため。
- **7-2.6（セッション失効）は 7-5 の後**。ガード付きページでの見せ方を決める issue だが、
  7-5 が最初のガード付きページなので、それまで実物で確認できない。
