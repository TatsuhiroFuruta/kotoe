...
- **7-3 の前に置く理由**：7-3 の描写入力には「保存」（下書き作成）があり、**押しても画面が変わらない**。
...
- 先に決めること：
  ...
  2. **成功以外の種別を持つか**（success / error / info）。7-3 の生成失敗をここで出すなら要る。
- タスク：
...
- 消費する予定の issue：7-3（保存した／生成を開始した）、7-4（通報しました）、7-5（お題を投稿しました）、
  7-6（削除しました）、7-2.6（セッション失効）。
- 完了条件：画面遷移を伴わない操作の後に通知が出て、数秒で消える。7-3 が「保存」で使える。
...
- 今どうなっているか（7-2 時点）：`api.ts` が「Authorization を載せたリクエストの 401」を検知して
  ...
  | 認証不要ページ（7-3 のお題詳細など） | 飛ばさない（意図的）。ヘッダーが黙ってユーザー名から「ログイン」に変わり、**押した操作は静かに失敗する**。長い描写文を書いて「保存」を押した場合が最悪 |
...
- **7-2 からの申し送り**：
  ...
  4. **「保存」の成功通知は 7-2.5 のフラッシュメッセージを使う**。押しても画面が変わらないため、
     通知が無いと保存されたか分からない。**この issue の前に 7-2.5 を済ませておくこと。**
  5. **失効（401）の扱いは 7-2.6 に委ねる**。この issue で初めて「認証不要ページで認証必須の操作を
     する」画面ができるので、描写文を書いている最中に失効すると操作が静かに失敗する。
...
## 進行順のまとめ（MVPの背骨）
`0-1 → 0-2 → 0-3 → 1-1 → 2-1 → 2-2 → 8-2a → 3-1 → 3-2 → 4-1 → 4-2 →（4-3）→ 5-1 → 5-2 → 6-1 → 6-3 → 4-4 → 7-1 → 7-2 → 7-2.5 → 7-3 →（7-4・7-5）→ 7-2.6 →（7-6）→ 8-1 → 8-2b`

- **7-2.5（フラッシュメッセージ）は 7-3 の前**。7-3 の「保存」は押しても画面が変わらないため、
  通知が無いと保存されたか分からない。
