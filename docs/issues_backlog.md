...
- 依存：なし（最初）
- タスク：
  - [ ] Rails 8 を `--api` で新規作成、Ruby/Rails バージョン固定
  - [ ] `docker-compose.yml`（rails / db(postgres) / next）を用意
  - [ ] DB 接続を環境変数化（`DATABASE_URL`、本番は Neon のプール接続文字列）
  - [ ] `bin/rails s` と Next.js dev が docker compose up で立ち上がる
...
- 依存：0-1
- タスク：
  - [ ] `CLAUDE.md` に技術構成・ディレクトリ方針・命名規約を記載
  - [ ] rubocop（`rubocop-rails-omakase`）導入、文字列はダブルクォート、`db/schema.rb` 等は除外
  - [ ] RSpec 導入（`rspec-rails`、FactoryBot）
...
- 依存：0-1
- タスク：
  - [ ] Next.js（App Router）+ TypeScript + TailwindCSS 初期化
  - [ ] 環境変数 `NEXT_PUBLIC_API_BASE_URL` を用意
  - [ ] トップに «疎通確認用» の最小ページ
- 完了条件：Next.js が起動し、Rails API に fetch して結果を表示できる。

### 🔵 0-4. CI（GitHub Actions：rubocop + rspec）
- 目的：main へのマージ前に自動チェックを回す。
- 依存：0-2
- タスク：
  - [ ] GitHub Actions で rubocop / rspec を実行するワークフロー
  - [ ] main への PR で必須チェックにする
- 完了条件：PR 作成時に rubocop / rspec が自動実行され、成功/失敗が表示される。
...
- タスク：
  - [ ] `discard` gem 導入
  - [ ] マイグレーション作成：`users` / `posts` / `attempts` / `likes` / `favorites` / `reports`
    - [ ] 各テーブル `t.timestamps`（created_at / updated_at）
    - [ ] `posts.discarded_at` / `attempts.discarded_at`（論理削除）
    - [ ] `attempts`：`description(text)` / `generated_image` / `similarity_score(integer, null許容)` / `status` / FK(post, user)
    - [ ] 外部キー制約と index、`likes(user_id, attempt_id)` / `favorites(user_id, post_id)` に複合ユニークindex
  - [ ] モデル関連付け（`has_many` / `belongs_to`）、`Discard::Model` を Post/Attempt に
  - [ ] バリデーション（必須項目、status の enum、ユニーク性）
  - [ ] モデル spec（関連・バリデーション・論理削除の絞り込み）
...
### 🟢 2-2. CORS 設定
- 目的：別ドメイン（Vercel↔Render）間の通信を許可する。
- 依存：2-1
- タスク：
  - [ ] `rack-cors` で許可オリジン（ローカル/本番）を環境変数化
  - [ ] Authorization ヘッダの露出設定
...
- 依存：1-1
- タスク：
  - [ ] Sidekiq または Solid Queue を導入・起動（docker compose に追加）
  - [ ] 動作確認用のダミージョブ
