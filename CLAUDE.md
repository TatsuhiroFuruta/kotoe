...
## 依存の脆弱性への対応
CI の bundler-audit は**脆弱性DBが更新された時点で落ちる**。コードを変えていなくても、ある日突然すべての PR が赤くなる。実装中の issue と切り離して扱う。

- **他の作業に混ぜず、単独の修正 PR を先に出す**。main を含む全ブランチのブロッカーになるため、Dependabot の週次PRは待たない。ブランチ名は `fix/<gem>-<version>` の形。
- **原則 `Gemfile.lock` のみ更新**し、`Gemfile` の制約は変えない（`bundle update <gem>`）。`~> 8.1.3` のような指定はパッチ更新を許すため、多くの場合そのままで足りる。制約自体を上げないと解決できないときだけ `Gemfile` を触る。
- **bundler-audit はローカル（開発コンテナ）では動かない**。脆弱性DBの取得に git が要るため。ローカルでは rspec / rubocop / brakeman と**アプリが実際に起動すること**まで確認し、脆弱性そのものの解消は**その PR の CI で確認する**。
- PR には **CVE 番号・影響する gem・Kotoe が実際にその機能を使っているか**を書く。使っていなくても CI が落ちる以上は更新するが、緊急度の判断材料になる。
- 前例：PR #52（rails-html-sanitizer の XSS）、PR #65（Rails 8.1.3.1 / CVE-2026-66066）。
...
- 論理削除は必ず `discard` を使う（`discarded_at`）。**物理削除しない**（他テーブルからの参照が壊れるため）。通常のクエリは `kept` スコープで未削除を対象にする。
  - **例外：トグル用の中間テーブル（`likes` / `favorites`）は物理削除する。** 理由は3つ。(1) `(user_id, 対象の id)` の複合ユニーク制約と両立せず、行を残すと同じ対象にいいね／お気に入りをし直せなくなる（`likes` は `(user_id, attempt_id)`、`favorites` は `(user_id, post_id)`）。(2) これらのテーブルには `discarded_at` が無く、`has_many ..., dependent: :destroy` も物理削除前提で書かれている。(3) 何からも参照されておらず、「取り消し」に記録を残す意味が無いため、上のルールの趣旨（参照が壊れる）が当てはまらない。
...
- **フロント単体テスト**：**MVP では導入しない**（フロントは表示と配線が中心で、単体テストが独自に守れる領域が薄いため）。ただし、UIから切り出せる純粋なロジック（例：生成ステータスのポーリング処理のカスタムフック、ユーティリティ関数）が生まれたら、**その部分にだけ Vitest** を後付けする（issue 7-1 で導入済み）。**Jest は採用しない**。React Testing Library でのコンポーネント結合テストも E2E と役割が被るため見送り。
  - 置き場所は **`frontend/test/`** に集約し、**`src/` の構造を写す**（`src/lib/auth/token-store.ts` → `test/lib/auth/token-store.test.ts`）。ソースへの併置ではなく分離ツリーにするのは、backend の `spec/` が `app/` を写しているのと揃えるため。対象の指定は `frontend/vitest.config.mts` の `include`。
  - `window` が無い状態（SSR）を検査するテストは、ファイル先頭に `// @vitest-environment node` を書いて node 環境に切り替える（既定は jsdom）。
