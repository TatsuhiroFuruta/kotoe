...
## 動作確認・デプロイの進め方
原則：**機能追加のたびに本番デプロイして確認しない**。確認は下の段階で行い、本番デプロイはマイルストーンの区切りだけ。
...
- そのため、本番デプロイ環境の構築（issue 8-2）を最初にやる必要はない。**MVP のコアループがローカルで動いてから**でよい。
- 例外：本番環境に依存する部分（Vercel↔Render の CORS/JWT、Neon への本番DB接続、画像生成APIの本番キー、カスタムドメインのDNS）は、デプロイ環境を作った直後に**最小の疎通確認**を一度しておく（ローカルでは再現しきれないため）。
